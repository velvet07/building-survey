...
export function getPhotoUrl(photo: Photo | string, thumbnail = false): string {
  ...
  // Legacy support for Supabase Storage (if file_path exists)
  if (typeof photo === 'object' && photo.file_path) {
    // This is legacy - should not be used for new photos
...
export async function downloadPhoto(photo: Photo): Promise<void> {
  ...
  // Legacy Supabase Storage download (should not be used for new photos)
  const url = getPhotoUrl(photo);
  const a = document.createElement('a');
  a.href = url;
  a.download = photo.file_name;
