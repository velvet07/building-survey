'use client';

import { User, USER_ROLE_LABELS, USER_ROLE_COLORS, USER_ROLE_DESCRIPTIONS } from '@/types/user.types';
import { Card } from '@/components/ui/Card';
import { Badge } from '@/components/ui/Badge';
import { Button } from '@/components/ui/Button';
import { formatDate } from '@/lib/utils';

export interface UserCardProps {
  user: User;
  onEdit: (user: User) => void;
  onDelete: (user: User) => void;
}

export function UserCard({ user, onEdit, onDelete }: UserCardProps) {
  return (
    <Card padding="md" className="group hover:border-primary-300 transition-colors">
      <div className="mb-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <h3 className="text-lg font-bold text-secondary-900 group-hover:text-primary-600 transition-colors leading-tight mb-1">
              {user.full_name || 'Nincs név megadva'}
            </h3>
            <p className="text-sm text-secondary-600 font-medium">{user.email}</p>
          </div>
          <div className="flex items-center justify-center w-12 h-12 rounded-full bg-primary-100 text-primary-600 font-bold text-lg">
            {(user.full_name || user.email).charAt(0).toUpperCase()}
          </div>
        </div>

        <div className="mb-3">
          <Badge variant={USER_ROLE_COLORS[user.role] as any} className="inline-block">
            {USER_ROLE_LABELS[user.role]}
          </Badge>
        </div>

        <div className="text-xs text-secondary-500 mb-2">
          <p className="italic">{USER_ROLE_DESCRIPTIONS[user.role]}</p>
        </div>

        <div className="text-xs text-secondary-500 space-y-1 font-medium">
          <p className="flex items-center gap-1.5">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Létrehozva: {formatDate(user.created_at)}
          </p>
          {user.updated_at !== user.created_at && (
            <p className="flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              Frissítve: {formatDate(user.updated_at)}
            </p>
          )}
        </div>
      </div>

      <div className="flex flex-wrap gap-2 pt-4 border-t border-secondary-200/80">
        <Button
          variant="secondary"
          size="sm"
          onClick={() => onEdit(user)}
          className="flex-1 md:flex-none"
        >
          <svg className="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
          </svg>
          Szerkesztés
        </Button>
        <Button
          variant="danger"
          size="sm"
          onClick={() => onDelete(user)}
          className="flex-1 md:flex-none"
        >
          <svg className="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
          </svg>
          Törlés
        </Button>
      </div>
    </Card>
  );
}
